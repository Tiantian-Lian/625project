...
# model selection
```{r}
full = glm(Diabetes_binary ~ ., data = train_data, family = binomial)
n = nrow(train_data)
bic_step = step(full, direction = "both", k = log(n), trace = 0)

formula(bic_step)
summary(bic_step)

set.seed(123)
folds = createFolds(train_data$Diabetes_binary, k = 5, returnTrain = TRUE)

vars = vector("list", 5)

for(i in 1:5){
  cv_train = train_data[folds[[i]], ]
  fit = glm(Diabetes_binary ~ ., data = cv_train, family = binomial)
  step_fit = step(fit, direction = "both", k = log(nrow(cv_train)), trace = 0)
  vars[[i]] = attr(terms(step_fit), "term.labels")
}

table(unlist(vars))


anova(bic_step, full, test="Chisq")
```
