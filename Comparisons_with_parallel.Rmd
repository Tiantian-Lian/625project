...
# 3. Random Forest with parallel computing
```{r}
library(pROC)
library(doParallel)
library(foreach)
...
res_list <- foreach(i = 1:5,
  ...
  auc_i   <- as.numeric(auc(roc(cv_valid$Diabetes_binary, rf_prob)))
  
  list(idx_valid = idx_valid, prob = rf_prob, auc = auc_i)
}
...
for (i in 1:5) {
  rf_cv_prob[res_list[[i]]$idx_valid] <- res_list[[i]]$prob
  auc_folds_rf[i] <- res_list[[i]]$auc
}
...
# 4. XGBoost with parallel computing
```{r}
library(xgboost)
library(parallel)
