...
Since 5-fold cross-validation for random forest and XGBoost ran quite slowly, we experimented with parallel computing. However, this did not noticeably reduce the elapsed time, likely due to the overhead of creating worker processes and copying large training subsets for each fold relative to the modest number of folds. In future work, we could explore alternative strategies to improve computational efficiency.
...
All team members contributed collaboratively to data analysis, model implementation, result interpretation, and report preparation, with each member taking primary responsibility for one modeling approach: Shuchen Wu for logistic regression, Tiantian Lian for XGBoost, and Meiqi Zhu for Random Forest.
